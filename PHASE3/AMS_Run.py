...
window.configure(background='#7EBFB3')
    ...
    def fill_attendance():
        ...
        if subb == '':
            ...
            def err_screen1():
                ...
                Button(errsc2, text='OK', command=del_errsc2, fg="white", bg="babyblue", width=9, height=1,
            ...
            def create_csv():
                ...
                csv_name = 'C:\Face-Recognition-Attendance-System\Attendance\Manually Attendance'+DB_table_name+'.csv'
            ...
            DATA_SUB = tk.Button(MFW, text="Enter Data", command=enter_data_DB, fg="black", bg="babyblue", width=20,
                                 height=2,
                                 activebackground="white", font=('times', 15, ' bold '))
            DATA_SUB.place(x=170, y=300)

            MAKE_CSV = tk.Button(MFW, text="Convert to CSV", command=create_csv, fg="black", bg="babyblue", width=20,
                                 height=2,
                                 activebackground="white", font=('times', 15, ' bold '))
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer /select,"C:\Face-Recognition-Attendance-System\Attendance\Manually Attendance\-------Check atttendance-------"')
    ...
    if l1 == '':
        ...
        try:
            ...
            detector = cv2.CascadeClassifier(
                'C:\Face-Recognition-Attendance-System\haarcascade_frontalface_default.xml')
            Enrollment = txt.get()
            Name = txt2.get()
            sampleNum = 0
            while (True):
            ...
            with open('C:\Face-Recognition-Attendance-System\StudentDetails\StudentDetails.csv', 'a+') as csvFile:
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    recognizer.read("C:\Face-Recognition-Attendance-System\TrainingImageLabel\Trainner.yml")
                except:
                ...
                harcascadePath = "C:\Face-Recognition-Attendance-System\haarcascade_frontalface_default.xml"
                faceCascade = cv2.CascadeClassifier(harcascadePath)
                df = pd.read_csv("C:\Face-Recognition-Attendance-System\StudentDetails\StudentDetails.csv")
                cam = cv2.VideoCapture(0)
                ...
                root.configure(background='grey80')
                cs = 'C:/Face-Recognition-Attendance-System/Attendance' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"C:\Face-Recognition-Attendance-System\Attendance\-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'pragya':
            if password == 'pragya123':
                ...
                cs = 'C:\Face-Recognition-Attendance-System\StudentDetails'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
def trainimg():
    ...
    global detector
    detector = cv2.CascadeClassifier("C:\Face-Recognition-Attendance-System\haarcascade_frontalface_default.xml")
    try:
        global faces, Id
        faces, Id = getImagesAndLabels("C:\Face-Recognition-Attendance-System\TrainingImage")
    except Exception as e:
        l = 'please make "TrainingImage" folder & put Images'
        Notification.configure(text=l, bg="",
                               width=50, font=('times', 18, 'bold'))
        Notification.place(x=350, y=400)

    recognizer.train(faces, np.array(Id))
    try:
        recognizer.save("C:\Face-Recognition-Attendance-System\TrainingImageLabel\Trainner.yml")
    except Exception as e:
...
message = tk.Label(window, text="Face-Recognition-Based-Attendance-Management-System", bg="#7EBFB3", fg="black", width=50,
                   height=3,activebackground="white",font=('times', 32, ' bold '))
...
lbl = tk.Label(window, text="Enter Enrollment : ", width=20, height=2,
               fg="white", bg="black", font=('times', 15, 'bold'))
...
lbl2 = tk.Label(window, text="Enter Name : ", width=20, fg="white",
                bg="black", height=2, font=('times', 15, ' bold '))
...
clearButton = tk.Button(window, text="Clear", command=clear, fg="white", bg="black",
                        width=10, height=1, activebackground="white", font=('times', 17, ' bold '))
...
AP = tk.Button(window, text="Check Registered students", command=admin_panel, fg="black",
               bg="Light Sea Green", width=19, height=1, activebackground="white", font=('times', 17, ' bold '))
AP.place(x=990, y=410)

takeImg = tk.Button(window, text="Take Images", command=take_img, fg="black", bg="Light Sea Green",
                    width=20, height=3, activebackground="white", font=('times', 17, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images", fg="black", command=trainimg, bg="Light Sea Green",
                     width=20, height=3, activebackground="white", font=('times', 17, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendance", fg="black", command=subjectchoose,
               bg="Light Sea Green", width=20, height=3, activebackground="white", font=('times', 17, ' bold '))
